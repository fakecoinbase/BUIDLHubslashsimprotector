...
contract SimProtector {
  ...
  function registerPhoneNumber(uint phoneNumber, address numberOwner) public {
    phoneNumberToAddress[phoneNumber] = numberOwner;
    emit RegisterPhoneNumber(phoneNumber, numberOwner, block.number);
  }
  ...
  function confirmShutdown(uint phoneNumber) public {
    ...
    emit ConfirmShutdown(phoneNumber, block.number);
  }

  function registerSimChange(uint phoneNumber) public {
    emit SimChange(phoneNumber, msg.sender, block.number);
  }
}
