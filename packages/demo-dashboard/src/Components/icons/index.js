...
export const Icon = props => {
    ...
    fa = fa.filter(f=>f !== 'fa-spin' && f !== 'fa-pulse' && f !== 'fa-fw')[0];
    if(spin) {
        spin = true;
    } 
    if(pulse) {
    ...
    let className = parts.filter(p=>!p.startsWith('fa-') && p!== 'fa' && p !== 'fas' && p !== 'fab').join(" ");
    if(fa) {
        //remove 'fa-' part
        fa = fa.substring(3);
        return (
            <i className={cn(className + " " + fa, align.allCenter, align.noMarginPad)}
...
export const IconBuilder = props => {
    ...
    if(classes.length > 1) {
        icon = (
            <div className={cn(align.allCenter, props.className, "fa-layers", "fa-fw", align.noMarginPad)}>
                {
                    classes.map((c,i)=>{
                        return (
                            <Icon key={i} className={cn(c)} />
                        )
                    })
                }
            </div>
